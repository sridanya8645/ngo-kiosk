...
let registrations = [
  {
    ...
    event_id: 1,
    event_name: 'Register for Newsletter and General Events',
    event_date: '2024-12-31',
    checked_in: 0,
    interested_to_volunteer: 'Yes',
    registered_at: '2024-07-30T10:00:00Z'
  },
  {
    id: 2,
    name: 'Jane Smith',
    ...
    event_date: '2024-12-31',
    checked_in: 1,
    checkin_date: '2024-07-30',
    interested_to_volunteer: 'No',
    registered_at: '2024-07-30T11:00:00Z'
  }
];
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const eventRow = events.find(e => e.id == event_id);
    ...
    const newRegistration = db.addRegistration({
      name, phone, email, event_id, 
      event_name: eventRow.name, 
      event_date: eventRow.date, 
      interested_to_volunteer: volunteerValue,
      checked_in: 0,
      registered_at: new Date().toISOString()
    });
    console.log('Registration inserted with ID:', newRegistration.id);
...
app.get('/api/registrations', async (req, res) => {
  try {
    ...
    const formattedRegistrations = registrations.map(r => ({
      ...
      interested_to_volunteer: r.interested_to_volunteer
    }));
...
app.post('/api/checkin', async (req, res) => {
  try {
    ...
    if (registration.event_name === 'Register for Newsletter and General Events') {
      const today = new Date().toISOString().split('T')[0];
      db.updateRegistration(registrationId, { checked_in: 1, checkin_date: today });
      res.json({ success: true, name: registration.name, eventName: registration.event_name });
      return;
    }

    // For all other events, check if registration is for today's event
    const todayEvents = db.getTodaysEvents();
    if (todayEvents.length === 0 || registration.event_id !== todayEvents[0].id) {
      console.warn('QR not valid for this event. Registration event_id:', registration.event_id, 'Today event id:', todayEvents[0] && todayEvents[0].id);
...
app.get('/api/raffle/winners', async (req, res) => {
  try {
    const winners = db.getRaffleWinners();
    res.json(winners);
  } catch (error) {
...
app.get('/api/raffle-winners', async (req, res) => {
  try {
    ...
    console.log('Raffle winners found:', formattedWinners.length);
